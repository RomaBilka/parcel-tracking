...
//59************
var start59 = regexp.MustCompile(`^59[\d]{12}$`)

//Starts with 20, length 14, only numbers
//20************
var start20 = regexp.MustCompile(`^20[\d]{12}$`)

//Starts with 1, length 14, only numbers
//1*************
var start1 = regexp.MustCompile(`^1[\d]{13}$`)
...
func (c *Carrier) Detect(trackId string) bool {
	if start59.MatchString(trackId) {
		return true
	}

	if start20.MatchString(trackId) {
		return true
	}

	if start1.MatchString(trackId) {
		return true
	}
